...
function isPerfectNum(a) {
  let summOfDividers = 0;

  for (let i = 0; i < a; i++ ) {
    a % i == 0 ? summOfDividers += i : false;
  }

  return summOfDividers == a ? true : false;
}

console.log(isPerfectNum(6));

//2
function showPerfectNumsInFange(min, max) {
  for (let i = min; i <= max; i++) {
    isPerfectNum(i) ? console.log(`${i} is perfect`) : false;
  }
}

showPerfectNumsInFange(1,10000)